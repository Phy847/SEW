...
// a) isEven: Die isEven Funktion prüft ob es sich bei einem
...
// Hinweis: Verwenden Sie die isEven Funkion

function filterEvenValues(values) {
    let evenValues = [];
...
// Funktionsaufruf
console.log("exercise 1: ", ...filterEvenValues([5, 3, 2, 1, 4, 6, 9]));
console.log("\n");
...
function concat(values1, values2) {
    ...
    for (let i = 0; i < values1.length; i++) {
        newValues.push(values1[i]);
    }
    for (let i = 0; i < values2.length; i++) {
        newValues.push(values2[i]);
    }
    return newValues;
}
...
hitCount = fireWeapon(5, avengerBoltCaonnon);
